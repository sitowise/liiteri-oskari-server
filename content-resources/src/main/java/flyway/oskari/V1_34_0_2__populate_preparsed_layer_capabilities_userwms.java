...
public class V1_34_0_2__populate_preparsed_layer_capabilities_userwms implements JdbcMigration {

    private static final Logger LOG = LogFactory.getLogger(V1_34_0_2__populate_preparsed_layer_capabilities_userwms.class);
    ...
    private void updateLayerCaps(int layerId, JSONObject capabilities, Connection conn)
            throws SQLException {
        final String sql = "UPDATE oskari_user_maplayer SET capabilities=? where id=?";
    ...
    List<OskariLayer> getLayers(Connection conn)
        ...
        // only process wms-layers
        final String sql = "SELECT id, url, type, name FROM oskari_user_maplayer where type='wmslayer' and capabilities='{}'";
        try (PreparedStatement statement = conn.prepareStatement(sql)) {
            try (ResultSet rs = statement.executeQuery()) {
                while (rs.next()) {
